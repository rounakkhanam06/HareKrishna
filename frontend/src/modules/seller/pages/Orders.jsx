...
const Orders = () => {
    ...
    useEffect(() => {
        if (isDetailsModalOpen || isQuickViewModalOpen) {
    ...
    const fetchOrders = async (requestedPage = 1, showPageLoader = false) => {
        try {
            ...
            // Map active tab to statusParam for backend filtering
            const statusToMatch = activeTab === 'Out for Delivery' ? 'out-for-delivery' : activeTab.toLowerCase();
            if (statusToMatch !== 'all') {
                params.status = statusToMatch;
            }
            ...
            const formattedOrders = (rawOrders || []).map(order => {
                const isValidDate = order.createdAt && !isNaN(Date.parse(order.createdAt));
                return {
                    id: order.orderId || order._id || 'Unknown',
                    _id: order._id,
                    customer: {
                        name: order.customer?.name || order.address?.name || 'Unknown',
                        phone: order.customer?.phone || order.address?.phone || '',
                        avatar: (order.customer?.name || order.address?.name || 'U').charAt(0)
                    },
    ...
    const handleStatusUpdate = async (orderId, newStatus) => {
        // Prevent updating terminal-state orders
        const currentOrder = filteredOrders.find(o => o.id === orderId);
        if (currentOrder && TERMINAL_STATUSES.includes(currentOrder.status?.toLowerCase())) {
            showToast(`Cannot change status of a ${currentOrder.status} order`, "error");
            return;
        }
        try {
            await sellerApi.updateOrderStatus(orderId, { status: newStatus.toLowerCase() });
            showToast(`Order status updated to ${newStatus}`, "success");
            await fetchOrders(page); // Refresh orders for the current page
                ...
                <>
                                            ...
                                            <div className="flex items-start justify-between gap-3">
                                                ...
                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                    ...
                                                    <select
                                                        ...
                                                        disabled={TERMINAL_STATUSES.includes(order.status?.toLowerCase())}
                                                        className={cn(
                                                            "w-full min-w-[100px] text-[10px] pl-2 pr-6 py-1.5 rounded-lg font-black uppercase appearance-none border outline-none",
                                                            TERMINAL_STATUSES.includes(order.status?.toLowerCase()) ? "cursor-not-allowed opacity-70" : "cursor-pointer",
                    ...
                    <AnimatePresence>
                        {isQuickViewModalOpen && (
                            <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4" onWheel={e => e.stopPropagation()}>
                                <motion.div
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    exit={{ opacity: 0 }}
                    ...
                    <AnimatePresence>
                        {isDetailsModalOpen && selectedOrder && (
                            <div className="fixed inset-0 z-[100] flex items-stretch sm:items-center justify-center p-3 sm:p-6 lg:p-12" onWheel={e => e.stopPropagation()}>
                                <motion.div
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    exit={{ opacity: 0 }}
                                        ...
                                        <div className="flex gap-2 items-center">
                                            ...
                                            <div className="relative inline-block w-40">
                                                <select
                                                    ...
                                                    disabled={TERMINAL_STATUSES.includes(selectedOrder.status?.toLowerCase())}
                                                    className={cn(
                                                        ...
                                                        TERMINAL_STATUSES.includes(selectedOrder.status?.toLowerCase()) ? "cursor-not-allowed opacity-70" : "cursor-pointer",
                                                ...
                                                {!TERMINAL_STATUSES.includes(selectedOrder.status?.toLowerCase()) && (
                                                    <HiOutlineChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 pointer-events-none opacity-60" />
                                                )}
                                            </div>
                                        </div>
