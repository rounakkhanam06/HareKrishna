...
const CashCollection = () => {
    ...
    const confirmSettlement = async () => {
        const amt = Number(settlementData.amount);
        if (isNaN(amt) || amt < 0) {
        ...
        const maxAllowed = Number(settlementData.rider?.currentCash || 0);
        if (amt > maxAllowed) {
            toast.error(`Settlement amount cannot exceed the rider's current cash in hand (₹${maxAllowed}).`);
            return;
        }
        try {
                                        ...
                                        <td className="px-6 py-6 text-right pr-8">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        try {
                                                            await adminApi.remindRiderCash(rider.id);
                                                            toast.success(`Settlement reminder notification sent to ${rider.name} successfully.`);
                                                        } catch (err) {
