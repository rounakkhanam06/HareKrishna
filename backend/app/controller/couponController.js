    ...
    try {
        ...
        if (req.user && req.user.role !== "admin") {
            ...
            for (const coupon of coupons) {
                if (coupon.perUserLimit && Number.isFinite(Number(coupon.perUserLimit)) && Number(coupon.perUserLimit) > 0) {
            ...
            filteredCoupons = eligibleCoupons;
        }

        return handleResponse(res, 200, "Coupons fetched successfully", filteredCoupons);
    } catch (error) {
...
const validateCouponData = (data) => {
    ...
    if (data.minOrderValue !== undefined && Number(data.minOrderValue) < 0) {
        throw new Error("Minimum order requirement cannot be negative");
    }
    if (data.maxDiscount !== undefined && data.maxDiscount !== null && Number(data.maxDiscount) < 0) {
    ...
    if (data.perUserLimit !== undefined && data.perUserLimit !== null && Number(data.perUserLimit) < 1) {
...
export const toggleCouponStatus = async (req, res) => {
    try {
        ...
        const nextIsActive = req.body?.isActive !== undefined ? Boolean(req.body.isActive) : !coupon.isActive;
        coupon.isActive = nextIsActive;
        await coupon.save();
        return handleResponse(res, 200, `Coupon status updated to ${nextIsActive ? 'active' : 'inactive'}`, coupon);
    } catch (error) {
        return handleResponse(res, 500, error.message);
    }
};
...
export const validateCoupon = async (req, res) => {
    try {
        ...
        if (customerId) {
            ...
            // Query direct Order collection for coupon usage
