...
const SellerTransactions = () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                const mapped = data.map(t => ({
                    id: 'TXN-' + t._id.slice(-8).toUpperCase(),
                    rawId: t._id,
                    referenceId: t.reference || 'N/A',
    ...
    const handleExport = () => {
        ...
        try {
            ...
            const headers = ['Transaction ID', 'Order ID', 'Date', 'Shop Name', 'Type', 'Amount (Rs.)', 'Commission (Rs.)', 'Tax (Rs.)', 'Status', 'Payment Method'];
                ...
                {selectedTxn && (
                    <div className="ds-section-spacing">
                        <div className="flex flex-col items-center text-center p-6 bg-slate-50 rounded-xl border border-slate-100">
                            ...
                            <p className="text-xs font-bold text-slate-400 uppercase mt-2 break-all px-4">{selectedTxn.id}</p>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="pt-4 flex gap-3">
                                <button
                                    onClick={async () => {
                                        try {
                                            ...
                                            doc.setFont(undefined, 'normal');
                                            doc.text(selectedTxn.id || 'N/A', margin + 55, y, { maxWidth: pageWidth - margin - (margin + 55) });
                                            y += 10;
                                            ...
                                            const shortSafeId = safeId.length > 12 ? `${safeId.slice(0, 8)}...${safeId.slice(-4)}` : safeId;
                                            doc.text(`Generated on ${new Date().toLocaleString()} • Voucher ID: ${shortSafeId}`, pageWidth / 2, y, { align: 'center' });
                                ...
                                <button
                                    title={selectedTxn.orderId ? "Go to Order Details" : "Copy Transaction ID"}
                                    onClick={() => {
                                        if (selectedTxn.orderId) {
                                            navigate(`/admin/orders/view/${selectedTxn.orderId}`);
                                            setSelectedTxn(null);
                                        } else {
                                            navigator.clipboard.writeText(selectedTxn.rawId || selectedTxn.id);
