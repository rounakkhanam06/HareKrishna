...
export const submitReview = async (req, res) => {
    try {
        ...
        // Only users who purchased and received the product should be allowed to rate
        ...
        if (!hasPurchased) {
            return handleResponse(res, 400, "You can only rate products that you have purchased and received");
        }
...
// Get approved reviews for a product (Public + user's own pending reviews)
export const getProductReviews = async (req, res) => {
    try {
        const { productId } = req.params;
        const userId = req.user?.id;
        const query = userId
            ? { productId, $or: [{ status: "approved" }, { userId }] }
