...
const CheckoutCouponSection = React.memo(function CheckoutCouponSection({
  ...
  return (
    <>
      ...
      <motion.div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        ...
        {coupons.length === 0 ? (
          ...
          <div className="flex gap-3 overflow-x-auto pt-1 pb-3 no-scrollbar -mx-5 px-5 snap-x items-stretch">
            {coupons.map((coupon) => {
              const isApplied = selectedCoupon?.code === coupon.code;
              return (
                <div
                  key={coupon.code}
                  className={`flex-shrink-0 w-[210px] min-h-[165px] snap-start rounded-2xl border-2 border-dashed p-3.5 flex flex-col gap-2 transition-all ${
                    isApplied
                      ? "border-[#153628] bg-[#153628]/10"
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-xs font-black px-2 py-0.5 rounded-lg tracking-widest uppercase ${
                        isApplied
                          ? "bg-[#153628]/20 text-[#153628]"
                          : "bg-orange-100 text-orange-600"
                      }`}>
                      {coupon.code}
                    </span>
                    {isApplied && (
                      <span className="text-[10px] font-black text-[#153628] uppercase tracking-wide">
                        ✓ Applied
                      </span>
                    )}
                  </div>
                  ...
                  {isApplied ? (
                    <button
                      ...
                      className="mt-auto w-full py-2 rounded-xl text-xs font-black bg-red-50 text-red-500 hover:bg-red-100 active:scale-95 transition-all">
                      Remove
                    </button>
                  ) : (
                    <button
                      onClick={() => onApplyCoupon(coupon)}
                      className="mt-auto w-full py-2 rounded-xl text-xs font-black bg-primary text-primary-foreground hover:bg-[var(--brand-400)] active:scale-95 transition-all">
                      Apply
                    </button>
                  )}
                </div>
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 rounded-xl font-bold text-sm transition-all ${
                        selectedCoupon?.code === coupon.code
                          ...
                          : "bg-primary text-primary-foreground hover:bg-[#0d231a]"
                      }`}>
