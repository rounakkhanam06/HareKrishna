...
// Document upload (rider uploads their own documents)
router.post("/documents", verifyToken, allowRoles("delivery"), upload.single("file"), uploadDeliveryDocument);

// Admin – verify or reject individual document
// These sit on the delivery router so they share the same prefix /delivery
// but require admin role
router.patch("/documents/admin/:id/verify/:docKey", verifyToken, allowRoles("admin"), adminVerifyDocument);
router.patch("/documents/admin/:id/reject/:docKey", verifyToken, allowRoles("admin"), adminRejectDocument);
