...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (response.data.success && response.data.result) {
        const list = response.data.result.notifications || response.data.result.items || response.data.results || [];
  ...
  useEffect(() => {
    ...
    return onDeliveryBroadcastWithdrawn(getToken, (payload) => {
      ...
      setNotifications((current) => {
        ...
        return current.filter((notification) => notification?.data?.orderId !== orderId);
      });
  ...
  const handleMarkAsRead = async (id) => {
    try {
      ...
      setNotifications((current) => {
        if (!Array.isArray(current)) return [];
        return current.map(n => n._id === id ? { ...n, isRead: true } : n);
      });
    } catch (error) {
