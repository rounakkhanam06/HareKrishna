...
export const createCategory = async (req, res) => {
  try {
    ...
    // Final sanity check: check both name and slug for duplicates
    const existingByName = await Category.findOne({ name: { $regex: `^${categoryData.name}$`, $options: 'i' } }).lean();
    if (existingByName) {
        return handleResponse(res, 400, `A category named "${categoryData.name}" already exists. Please choose a different name.`);
    }
    const existingBySlug = await Category.findOne({ slug: categoryData.slug }).lean();
    if (existingBySlug) {
        return handleResponse(res, 400, `A category named "${existingBySlug.name}" already exists with the same URL slug. Please choose a different name.`);
    }
...
export const updateCategory = async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      ...
      const msg = dupField === 'name'
        ? `A category named "${dupValue}" already exists. Please choose a different name.`
        : `A category with this name already exists (slug: "${dupValue}"). Please choose a different name.`;
      return handleResponse(res, 400, msg);
    }
    if (error?.name === "ValidationError" || error?.name === "CastError") return handleResponse(res, 400, error.message);
    return handleResponse(res, 500, `Category operation failed: ${error.message}`);
  }
...
export const deleteCategory = async (req, res) => {
  try {
    ...
    // Guard: reject invalid / stringified-object IDs immediately
    if (!id || !mongoose.Types.ObjectId.isValid(id)) {
      return handleResponse(res, 400, `Invalid category ID: "${id}"`);
    }
