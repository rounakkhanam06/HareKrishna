...
async function startup() {
  try {
    ...
    // Step 6.5: Backfill Rider IDs for existing delivery partners
    try {
      const Delivery = mongoose.model("Delivery");
      const ridersWithoutId = await Delivery.find({
        $or: [{ riderId: { $exists: false } }, { riderId: null }],
      });
      if (ridersWithoutId.length > 0) {
        console.log(`[Startup] Found ${ridersWithoutId.length} delivery partners without a unique Rider ID. Backfilling...`);
        for (const rider of ridersWithoutId) {
          // Trigger the pre-save hook to generate and assign a unique riderId
          await rider.save();
        }
        console.log(`[Startup] Rider ID backfill completed.`);
      }
    } catch (error) {
      console.error('[Startup] Warning: Rider ID backfill failed:', error.message);
    }
