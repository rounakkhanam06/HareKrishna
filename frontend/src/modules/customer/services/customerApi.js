...
export const customerApi = {
  ...
  requestReturn: (orderId, data) => {
    invalidateCache("/orders");
    return axiosInstance.post(`/orders/${orderId}/returns`, data);
  },
  rateDeliveryPartner: (orderId, data) => {
    invalidateCache("/orders");
    return axiosInstance.post(`/orders/${orderId}/rate-delivery`, data);
  },
  getDeliveryRating: (orderId) =>
    getWithDedupe(`/orders/${orderId}/delivery-rating`, {}, { ttl: 0 }),
  getReturnDetails: (orderId) =>
    axiosInstance.get(`/orders/${encodeURIComponent(String(orderId ?? "").trim())}/returns`),
  
  // Product Reviews
  getProductReviews: (productId, params) =>
    getWithDedupe(`/products/${productId}/reviews`, params),
  getProductReviewSummary: (productId) =>
    getWithDedupe(`/products/${productId}/review-summary`, {}, { ttl: 5000 }),
  getOrderProductReviews: (orderId) =>
    getWithDedupe(`/orders/${orderId}/product-reviews`, {}, { ttl: 0 }),
  submitProductReview: (data) => {
    invalidateCache("/orders");
    return axiosInstance.post("/product-reviews", data);
  },
  updateProductReview: (reviewId, data) => {
    invalidateCache("/orders");
    return axiosInstance.patch(`/product-reviews/${reviewId}`, data);
  },
