...
export async function hydrateOrderItems(
  ...
  // Verify sellers' status
  const sellerIds = [...new Set(products.map((p) => String(p.sellerId)).filter(Boolean))];
  const Seller = (await import("../../models/seller.js")).default;
  const activeSellers = await Seller.find({ _id: { $in: sellerIds }, isActive: true, isSuspended: { $ne: true } })
    .select("_id")
    .lean();
  ...
  return orderItems.map((item) => {
    ...
    if (!activeSellerIdSet.has(String(product.sellerId))) {
      throw new Error(`Product is from an inactive or suspended store: ${product.name}`);
    }
