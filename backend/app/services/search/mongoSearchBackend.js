...
export class MongoSearchBackend extends SearchBackend {
  ...
  async search(query) {
    ...
    try {
      ...
      if (mongoQuery.sellerId) {
        if (mongoQuery.sellerId.$in) {
          const filtered = mongoQuery.sellerId.$in.map(String).filter((id) => activeSellerIds.includes(id));
          ...
          mongoQuery.sellerId = { $in: filtered };
        } else {
          const sid = String(mongoQuery.sellerId);
        ...
        mongoQuery.sellerId = { $in: activeSellerIds };
      }
