...
export const markOrderDeliveredAndSettle = async (req, res) => {
  try {
    ...
    if (
      ...
      finalUpdated = await handleCodOrderFinance(updated._id, {
        deliveryPartnerId,
        actorId: req.user?.id || null,
      });
    }
    ...
    emitOrderStatusUpdate(
      finalUpdated.orderId,
      { workflowStatus: "DELIVERED", status: "delivered" },
      finalUpdated.customer
    );

    emitNotificationEvent(NOTIFICATION_EVENTS.ORDER_DELIVERED, {
      orderId: finalUpdated.orderId,
      customerId: finalUpdated.customer,
      userId: finalUpdated.customer,
      deliveryId: finalUpdated.deliveryBoy,
      sellerId: finalUpdated.seller,
    });

    if (updated?.paymentMode === "COD" && updated?.deliveryBoy && !updated?.financeFlags?.codMarkedCollected) {
      return handleResponse(res, 200, "Order delivered and COD cash collected", finalUpdated);
    }
    return handleResponse(res, 200, "Order delivered and settlement queued", finalUpdated);
  } catch (error) {
    return handleResponse(res, error.statusCode || 500, error.message);
  }
};
