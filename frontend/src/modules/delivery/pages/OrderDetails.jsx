...
const OrderDetails = () => {
    ...
    try {
      ...
      if (order?.returnStatus && order.returnStatus !== "none") {
        ...
        try {
          ...
          console.warn("GPS failed, falling back to dummy coordinates", err);
          const sellerCoords = order?.seller?.location?.coordinates;
          location = Array.isArray(sellerCoords) && sellerCoords.length >= 2
            ? { lat: sellerCoords[1], lng: sellerCoords[0] }
            : { lat: 12.9716, lng: 77.5946 };
          toast.warning("GPS is disabled. Using fallback store location.");
        }
  ...
  const handleOtpValidationSuccess = (data) => {
    ...
    if (isReturn) {
      ...
      setStep(4);
      if (updatedOrder) {
        setOrder((prev) => ({ ...(prev || {}), ...updatedOrder, status: "delivered", workflowStatus: "DELIVERED" }));
      } else {
        setOrder((prev) => prev ? { ...prev, status: "delivered", workflowStatus: "DELIVERED" } : prev);
      }
      window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  const totalBill = order.paymentBreakdown?.grandTotal ?? order.pricing?.total ?? 0;
  const walletAmount = order.paymentBreakdown?.walletAmount ?? order.pricing?.walletAmount ?? 0;
  const collectCash = Math.max(0, totalBill - walletAmount);
  const discountTotal = order.paymentBreakdown?.discountTotal ?? order.pricing?.discount ?? 0;
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar-dark text-left">
                    {(order.returnItems || order.items)?.map((item, i) => {
                      const itemImg =
                        ...
                        order.items?.find((orig) => String(orig.product?._id || orig.product) === String(item.product?._id || item.product) || orig.name === item.name)?.image ||
                        order.items?.[item.itemIndex || i]?.image ||
                        order.items?.[item.itemIndex || i]?.product?.mainImage ||
                        "/placeholder.png";
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-0.5 flex-wrap gap-y-1">
                        ...
                        <p className="text-[10px] text-gray-400 font-medium">Bill: Rs.{totalBill}</p>
                        <p className="text-[10px] text-brand-700 font-bold bg-brand-50 border border-brand-200 px-1.5 py-0.5 rounded">
                          Earnings: ₹{Math.round(isReturn ? (order.returnDeliveryCommission || 0) : (order.paymentBreakdown?.riderPayoutTotal || totalBill * 0.1))}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            {itemsExpanded && (
              ...
              >
                <div className="p-4 border-t border-gray-100 bg-gray-50/50 space-y-3">
                  ...
                  {discountTotal > 0 && (
                    <div className="flex justify-between items-center text-sm text-brand-600 font-bold bg-brand-50/50 -mx-1 px-1 py-0.5 rounded">
                  ...
                  <div className="pt-3 mt-2 border-t border-gray-200 flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold text-gray-900">Rs.{totalBill}</span>
                  </div>
                </div>
              </motion.div>
            )}
