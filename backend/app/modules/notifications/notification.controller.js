...
function resolveNotificationFilter(req) {
  ...
  if (!rawId) return { $or: [] };
  const userObjectId = mongoose.Types.ObjectId.isValid(String(rawId))
    ? new mongoose.Types.ObjectId(String(rawId))
    : null;
  const ids = [rawId, userObjectId, String(rawId)].filter(Boolean);
  return {
    $or: [
      { userId: { $in: ids } },
      { recipient: { $in: ids } },
...
export const markNotificationsRead = async (req, res) => {
  try {
    ...
    const baseFilter = {
      $or: [{ userId: { $in: ids } }, { recipient: { $in: ids } }],
    };
    ...
    const filter =
      markAll || (!notificationId && notificationIds.length === 0 && !req.params?.id)
        ? { ...baseFilter }
        : {
            _id: { $in: validIds },
            ...baseFilter,
