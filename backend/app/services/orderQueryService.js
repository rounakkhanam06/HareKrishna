...
export function buildSellerOrdersQuery({
  ...
  if (paymentMethod) {
    if (paymentMethod === "cash") {
      withStatus["payment.method"] = "cash";
    } else if (paymentMethod === "online") {
      withStatus["payment.method"] = { $in: ["online", "wallet"] };
    }
  }
...
export async function fetchSellerOrdersPage({
  ...
  const sortOrder = sortBy === "oldest" ? { createdAt: 1, _id: 1 } : { createdAt: -1, _id: -1 };
    ...
    async () => {
      const [orders, total] = await Promise.all([
        Order.find({ customer: customerId })
          .select(
            "orderId checkoutGroupId customer seller items address payment pricing status workflowStatus workflowVersion returnStatus timeSlot createdAt deliveryBoy",
          )
          .sort({ createdAt: -1, _id: -1 })
          .skip(skip)
          .limit(limit)
