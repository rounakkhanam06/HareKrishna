      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-4xl items-center justify-center px-4 py-10">
        ...
        >
          <div className="mb-8 flex items-center justify-between gap-4">
            ...
            >
              {isSuspended || isRejected ? <ShieldAlert className="h-4 w-4" /> : <Clock3 className="h-4 w-4" />}
              {isSuspended ? "Account Suspended" : isRejected ? "Application Rejected" : "Application Pending"}
            </div>
          </div>
          ...
          <p className="mt-4 text-base md:text-lg text-slate-200/90 font-medium max-w-2xl">
            {isSuspended
              ? "Your seller account has been suspended due to violations of our platform terms of service. Please contact admin support."
              : isRejected
          ...
          {isSuspended ? (
            <div className="mt-6 rounded-2xl border border-rose-300/25 bg-rose-400/10 px-4 py-3 text-sm text-rose-100 flex items-start gap-3">
              <ShieldAlert className="h-5 w-5 mt-0.5 shrink-0 text-rose-400" />
              <p className="font-semibold">
                Your seller account is suspended. Please contact platform support.
              </p>
            </div>
          ) : rejectionReason ? (
          ...
          {!isRejected && !isSuspended ? (
            <div className="mt-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100 flex items-start gap-3">
