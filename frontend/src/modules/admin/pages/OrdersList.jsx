...
const OrdersList = () => {
    ...
    const fetchOrders = async (requestedPage = 1) => {
        ...
        try {
            ...
            if (filterPayment !== 'all') {
                params.paymentMethod = filterPayment === 'COD' ? 'cash' : 'online';
            }
            if (sortBy) {
            ...
            if (response.data.success) {
                ...
                const formatted = dbOrders.map(o => ({
                    id: o.orderId || 'UNSET',
                    _id: o._id,
                    customer: o.customer?.["Farmer Name"] || o.customer?.name || o.address?.name || 'Customer',
                    ...
                    payment: o.payment?.method === 'cash' ? 'COD' : 'Digital',
                }));
                setOrders(formatted);
                setSummary({
    ...
    }, [pageSize, status, searchTerm, dateRange, filterPayment, sortBy]);
    ...
    const filteredOrders = useMemo(() => {
        ...
        return result;
    }, [safeOrders, searchTerm, status, filterPayment, sortBy]);
    ...
    const handleExport = () => {
        ...
        const url = URL.createObjectURL(blob);
        link.setAttribute('href', url);
        link.setAttribute('download', `eannadata-orders-${status}-${new Date().toISOString().split('T')[0]}.csv`);
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={cn(
                                    ...
                                    isFilterOpen ? "bg-fuchsia-50 text-fuchsia-600 ring-1 ring-fuchsia-100" : "bg-slate-50 text-slate-400 hover:text-slate-600"
                                )}
                            >
                                ...
                                {isFilterOpen && (
                                    <>
                                        ...
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0, scale: 1 }}
                                            exit={{ opacity: 0, y: 10, scale: 0.95 }}
                                            className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl ring-1 ring-slate-100 p-4 z-20 space-y-4 text-left"
                                        >
                                            <div>
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Payment Method</label>
                                                <div className="flex gap-2">
                                                    {['all', 'COD', 'Digital'].map((m) => (
                                                        <button
                                                            key={m}
                                                            onClick={() => setFilterPayment(m)}
                                                            className={cn(
                                                                "px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border",
                                                                filterPayment === m ? "bg-fuchsia-50 text-fuchsia-600 border-fuchsia-200" : "bg-white text-slate-500 border-slate-100 hover:bg-slate-50"
                                                            )}
                                                        >
                                                            {m === 'all' ? 'All' : m}
                                                        </button>
                                            ...
                                            <div className="pt-3 border-t border-slate-100">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Sort Order</label>
                                                <div className="flex gap-2">
                                                    {['newest', 'oldest'].map((s) => (
                                                        <button
                                                            key={s}
                                                            ...
                                                            className={cn(
                                                                "px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border",
                                                                sortBy === s ? "bg-fuchsia-50 text-fuchsia-600 border-fuchsia-200" : "bg-white text-slate-500 border-slate-100 hover:bg-slate-50"
                                                            )}
                                                        >
                                                            {s}
                                                        </button>
