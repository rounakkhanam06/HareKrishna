...
import { sendSellerVerificationOtpEmail, sendSellerPasswordResetOtpEmail, useRealEmailOTP } from "./emailService.js";

const SELLER_SIGNUP_PURPOSE = "seller_signup";
const SELLER_PASSWORD_RESET_PURPOSE = "seller_password_reset";
const SELLER_PASSWORD_RESET_VERIFIED_PURPOSE = "seller_password_reset_verified";
const PASSWORD_RESET_TOKEN_EXPIRY = "10m";
...
async function dispatchPhoneOtp({ phone, otp }) {
  if (useRealSMS()) {
    console.log(`[SellerPhoneOTP][real] phone: ${phone} -> OTP: ${otp}`);
...
/* ============================================================
   ...
   Reuses: OtpVerification model, rate limiting, email dispatch
============================================================= */

export async function issuePasswordResetOtp({ email, ipAddress = "unknown" }) {
  const target = normalizeEmail(email);

  // Check if seller exists — do NOT reveal to the caller whether email was found
  ...
  if (!seller) {
    ...
    console.log(
      JSON.stringify({
        level: "info",
        ts: new Date().toISOString(),
        event: "seller_password_reset_otp_no_account",
  ...
  const sendAllowed = await incrementWindowCounter(
    `seller:otp:send:email:${target}:password_reset`,
    {
      limit: OTP_SEND_LIMIT_PER_WINDOW(),
      windowSeconds: OTP_SEND_LIMIT_WINDOW_SECONDS(),
    },
  ...
  if (!session) {
    session = new OtpVerification({
      ...
      channel: "email",
      target,
      otpHash: hashOtp("email", target, otp),
      expiresAt,
      verifiedAt: null,
      failedAttempts: 0,
      lastSentAt: now,
    ...
    session.otpHash = hashOtp("email", target, otp);
    session.expiresAt = expiresAt;
    session.verifiedAt = null;
    session.failedAttempts = 0;
    session.lastSentAt = now;
...
export async function verifyPasswordResetOtp({ email, otp, ipAddress = "unknown" }) {
  const target = normalizeEmail(email);
  const code = String(otp || "").trim();
  ...
  const verifyAllowed = await incrementWindowCounter(
    `seller:otp:verify:email:${target}:password_reset`,
    {
      limit: OTP_VERIFY_LIMIT_PER_WINDOW(),
      windowSeconds: OTP_VERIFY_LIMIT_WINDOW_SECONDS(),
    },
  ...
  const isValid = hashOtp("email", target, code) === session.otpHash;
  if (!isValid) {
    session.failedAttempts = (session.failedAttempts || 0) + 1;
    await session.save();
  ...
  // Mark verified, then delete so it cannot be reused
  await OtpVerification.deleteOne({ _id: session._id });

  // Issue a short-lived reset token
  const resetToken = jwt.sign(
    {
      purpose: SELLER_PASSWORD_RESET_VERIFIED_PURPOSE,
      email: target,
    },
    verificationSecret(),
...
export function consumePasswordResetToken(resetToken) {
  ...
  try {
    ...
    const error = new Error("Invalid or expired reset session. Please request a new OTP.");
    error.statusCode = 400;
    throw error;
  }

  if (payload?.purpose !== SELLER_PASSWORD_RESET_VERIFIED_PURPOSE || !payload?.email) {
