...
const CouponManagement = () => {
    ...
    const handleToggleStatus = async (coupon) => {
        ...
        setCoupons(prev => prev.map(c => c._id === coupon._id ? { ...c, isActive: nextIsActive } : c));

        try {
            const res = await adminApi.updateCouponStatus(coupon._id, nextIsActive);
            if (res.data?.success) {
                showToast(`Coupon marked as ${nextIsActive ? 'Active' : 'Inactive'}`, 'success');
            }
        } catch (error) {
            // Revert on error
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (payload.maxDiscount !== undefined && payload.maxDiscount < 0) {
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="h-1.5 w-32 bg-slate-100 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    style={{ width: c.usageLimit ? `${((c.usedCount || 0) / c.usageLimit) * 100}%` : '100%' }}
                                                />
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-4 py-6 text-center">
                                        <div className="flex flex-col items-center gap-1.5">
                                            {(() => {
                                                ...
                                                const label = !c.isActive ? 'inactive' : isExpired ? 'expired' : 'active';
                                                const variant = !c.isActive ? 'gray' : isExpired ? 'error' : 'success';
                                                return (
                                            ...
                                            <button
                                                ...
                                                title={`Click to set as ${c.isActive ? 'Inactive' : 'Active'}`}
                                            >
                                                <div className={cn(
                                                ...
                                                <span className={cn(
                                                    ...
                                                    c.isActive ? "text-emerald-600" : "text-slate-500"
                                                )}>
