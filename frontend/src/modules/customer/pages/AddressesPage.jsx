...
import { Plus, Home, Briefcase, MapPin, Trash2, Edit2, ChevronLeft, Check } from 'lucide-react';
...
const AddressesPage = () => {
    ...
    const [isAddOpen, setIsAddOpen] = useState(() => {
        return sessionStorage.getItem('address_page_add_open') === 'true';
    });
    ...
    const [addForm, setAddForm] = useState(() => {
        const saved = sessionStorage.getItem('address_page_add_form');
        if (saved) {
            try {
    ...
    useEffect(() => {
        sessionStorage.setItem('address_page_add_open', isAddOpen);
        if (!isAddOpen) {
            sessionStorage.removeItem('address_page_add_form');
        }
    }, [isAddOpen]);

    useEffect(() => {
        if (isAddOpen) {
            sessionStorage.setItem('address_page_add_form', JSON.stringify(addForm));
        }
    }, [addForm, isAddOpen]);
    ...
    const handleSaveNewAddress = async () => {
        ...
        const getAddressKey = (a) => {
            const label = (a.label || a.type || 'home').toLowerCase().trim();
            const addressVal = (a.fullAddress || a.address || '').toLowerCase().trim();
            const landmarkVal = (a.landmark || '').toLowerCase().trim();
            const cityVal = (a.city || '').toLowerCase().trim();
            const stateVal = (a.state || '').toLowerCase().trim();
            const pincodeVal = (a.pincode || '').toLowerCase().trim();
            return `${label}|${addressVal}|${landmarkVal}|${cityVal}|${stateVal}|${pincodeVal}`;
        };

        const isDuplicate = rawAddresses.some(existing => getAddressKey(existing) === getAddressKey(newAddr));
        if (isDuplicate) {
        ...
        try {
            ...
            try {
                ...
                // Geocoding is best-effort — address saves fine without coordinates
                console.warn('Geocoding failed (non-blocking):', e?.response?.data?.message || e?.message);
            }
    ...
    const handleUpdateAddress = async () => {
        ...
        const isDuplicate = rawAddresses.some((existing, i) => i !== idx && getAddressKey(existing) === getAddressKey(updatedRaw));
        if (isDuplicate) {
            toast.error('This address is already saved');
            return;
        }
    ...
    const handleMakeDefault = async (addr, idx) => {
        if (idx === undefined) {
            idx = rawAddresses.findIndex(a => 
                (a._id === addr.id) || 
                (a.fullAddress === addr.address && (a.label || 'home').toLowerCase() === (addr.type || 'home').toLowerCase())
            );
        }
        if (idx <= 0) return; // Already default or not found

        const updatedAddresses = [...rawAddresses];
        const [target] = updatedAddresses.splice(idx, 1);
        updatedAddresses.unshift(target);
                    ...
                    {loading ? (
                        ...
                        <div key={addr.id} className="bg-white rounded-xl p-4 border border-slate-200 relative overflow-hidden">
                            ...
                            <div className="mt-4 flex items-center gap-2 pt-3 border-t border-slate-100">
                                {!addr.isDefault && (
                                    <button
                                        onClick={() => handleMakeDefault(addr, index)}
                                        className="flex-1 py-2 rounded-lg bg-slate-100 hover:bg-brand-50 hover:text-brand-700 text-slate-700 font-semibold text-xs transition-colors flex items-center justify-center gap-1.5 border border-transparent hover:border-brand-200"
                                    >
                ...
                <DialogContent className="sm:max-w-[425px] max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid gap-2">
                             ...
                             <Input id="name" placeholder="John Doe" value={addForm.name} onChange={e => { const v = e.target.value.replace(/[0-9]/g, ''); setAddForm(f => ({ ...f, name: v })); }} />
                         </div>
                         <div className="grid gap-2">
                             <Label htmlFor="phone">Phone Number</Label>
                             <Input id="phone" placeholder="+91 9876543210" inputMode="tel" value={addForm.phone} onChange={e => { const v = e.target.value.replace(/[^0-9+]/g, ''); setAddForm(f => ({ ...f, phone: v })); }} />
                         </div>
                         <div className="grid gap-2">
                         ...
                         <div className="grid grid-cols-2 gap-4">
                             <div className="grid gap-2">
                                 ...
                                 <Input id="city" placeholder="New Delhi" value={addForm.city} onChange={e => { const v = e.target.value.replace(/[0-9]/g, ''); setAddForm(f => ({ ...f, city: v })); }} />
                             </div>
                             <div className="grid gap-2">
                                 <Label htmlFor="state">State</Label>
                                 <Input id="state" placeholder="Delhi" value={addForm.state} onChange={e => { const v = e.target.value.replace(/[0-9]/g, ''); setAddForm(f => ({ ...f, state: v })); }} />
                             </div>
                         </div>
                         <div className="grid gap-2">
                             <Label htmlFor="pincode">Pincode</Label>
                             <Input id="pincode" placeholder="110075" inputMode="numeric" maxLength={6} value={addForm.pincode} onChange={e => { const v = e.target.value.replace(/[^0-9]/g, '').slice(0, 6); setAddForm(f => ({ ...f, pincode: v })); }} />
                         </div>
                    </div>
                ...
                <DialogContent className="sm:max-w-[425px] max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid gap-2">
                             ...
                             <Input id="edit-name" value={editForm.name} onChange={e => { const v = e.target.value.replace(/[0-9]/g, ''); setEditForm(f => ({ ...f, name: v })); }} />
                         </div>
                         <div className="grid gap-2">
                             <Label htmlFor="edit-phone">Phone Number</Label>
                             <Input id="edit-phone" inputMode="tel" value={editForm.phone} onChange={e => { const v = e.target.value.replace(/[^0-9+]/g, ''); setEditForm(f => ({ ...f, phone: v })); }} />
                         </div>
                         <div className="grid gap-2">
                         ...
                         <div className="grid grid-cols-2 gap-4">
                             <div className="grid gap-2">
                                 ...
                                 <Input id="edit-city" placeholder="New Delhi" value={editForm.city} onChange={e => { const v = e.target.value.replace(/[0-9]/g, ''); setEditForm(f => ({ ...f, city: v })); }} />
                             </div>
                             <div className="grid gap-2">
                                 <Label htmlFor="edit-state">State</Label>
                                 <Input id="edit-state" placeholder="Delhi" value={editForm.state} onChange={e => { const v = e.target.value.replace(/[0-9]/g, ''); setEditForm(f => ({ ...f, state: v })); }} />
                             </div>
                         </div>
                         <div className="grid gap-2">
                             <Label htmlFor="edit-pincode">Pincode</Label>
                             <Input id="edit-pincode" placeholder="110075" inputMode="numeric" maxLength={6} value={editForm.pincode} onChange={e => { const v = e.target.value.replace(/[^0-9]/g, '').slice(0, 6); setEditForm(f => ({ ...f, pincode: v })); }} />
                         </div>
                    </div>
                    <DialogFooter className="gap-2">
