...
export async function deliverNotificationById(notificationId) {
  ...
  // Deduplicate tokens and pick the most recently active tokens per platform
  const uniqueTokenMap = new Map();
  for (const tokenDoc of tokens) {
    const t = String(tokenDoc.token || "").trim();
    if (t && !uniqueTokenMap.has(t)) {
      uniqueTokenMap.set(t, tokenDoc);
    }
  }
  // Take top most recently used active token to prevent multi-token duplicate pushes on same device
  const targetTokenDocs = Array.from(uniqueTokenMap.values()).slice(0, 1);
  const targetTokens = targetTokenDocs.map((doc) => doc.token);
