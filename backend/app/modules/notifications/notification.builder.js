...
function eventDefinition(eventType) {
  switch (eventType) {
    ...
    case NOTIFICATION_EVENTS.PRODUCT_REVIEW_RECEIVED:
      return {
        multi: true,
        definitions: [
          {
            ...
            body: () => "Thank you for reviewing this product.",
          },
          {
            role: NOTIFICATION_ROLES.SELLER,
            recipientIds: (payload) => normalizeIdList(payload.sellerId),
    ...
    case NOTIFICATION_EVENTS.DELIVERY_RATING_RECEIVED:
      return {
        role: NOTIFICATION_ROLES.DELIVERY,
        recipientIds: (payload) => normalizeIdList(payload.deliveryPartnerId),
        title: () => "New Rating Received ⭐",
        body: (payload) =>
          `You received a new ${payload.stars}-star rating for Order #${payload.orderId}.` +
          (payload.review ? ` Preview: "${payload.review.substring(0, 40)}..."` : ""),
      };
    ...
    case NOTIFICATION_EVENTS.PRODUCT_NOTIFY_REQUEST:
      return {
        ...
        body: (payload) => {
          const productName = String(payload.productName || "Product").trim() || "Product";
          const variantSku = String(payload.variantSku || "").trim();
          const variantSuffix = variantSku ? ` (${variantSku})` : "";
          return `A customer has requested to be notified when ${productName}${variantSuffix} is restocked.`;
        },
      };
    ...
    case NOTIFICATION_EVENTS.DELIVERY_PROFILE_APPROVED:
      return {
        role: NOTIFICATION_ROLES.DELIVERY,
        recipientIds: (payload) => normalizeIdList(payload.deliveryId),
    ...
    case NOTIFICATION_EVENTS.DELIVERY_PROFILE_REJECTED:
      return {
        role: NOTIFICATION_ROLES.DELIVERY,
        recipientIds: (payload) => normalizeIdList(payload.deliveryId),
        title: () => "Application Update ❌",
        body: (payload) => `Your delivery partner application was rejected. Reason: ${payload.feedback || "Details provided do not meet platform criteria."}`,
      };
    case NOTIFICATION_EVENTS.SELLER_APPROVED:
      return {
        ...
        body: () => "Congratulations! Your seller registration has been approved. You can now manage your inventory and accept orders.",
      };
    case NOTIFICATION_EVENTS.SELLER_REJECTED:
      return {
        ...
        recipientIds: (payload) => normalizeIdList(payload.sellerId),
        title: () => "Application Update ❌",
        body: (payload) => `Your seller registration request was rejected. Reason: ${payload.feedback || "Details provided do not meet platform criteria."}`,
      };
    case NOTIFICATION_EVENTS.NEW_SELLER_REGISTRATION:
      return {
        ...
        body: (payload) => `${payload.shopName || "A new seller"} has registered and is pending approval.`,
      };
    case NOTIFICATION_EVENTS.NEW_DELIVERY_REGISTRATION:
      return {
        ...
        body: (payload) => `${payload.name || "A new rider"} has registered and is pending approval.`,
      };
    case NOTIFICATION_EVENTS.CASH_SETTLEMENT_REMINDER:
      return {
        role: NOTIFICATION_ROLES.DELIVERY,
        recipientIds: (payload) => normalizeIdList(payload.deliveryPartnerId),
        title: () => "Cash Settlement Reminder 🔔",
        body: (payload) => `Please settle your collected cash of ₹${payload.currentCash || 0}. Your limit is ₹${payload.limit || 5000}.`,
      };
    default:
      return null;
  }
...
function eventData(eventType, payload = {}, role) {
  ...
  if (eventType === NOTIFICATION_EVENTS.DELIVERY_PROFILE_APPROVED || eventType === NOTIFICATION_EVENTS.DELIVERY_PROFILE_REJECTED) {
    const baseUrl = getFrontendBaseUrl();
    return {
      eventType,
      link: `${baseUrl}/delivery/pending-approval`,
      ...(payload.data || {}),
    };
  }

  if (eventType === NOTIFICATION_EVENTS.SELLER_APPROVED || eventType === NOTIFICATION_EVENTS.SELLER_REJECTED) {
    const baseUrl = getFrontendBaseUrl();
    return {
      eventType,
      link: `${baseUrl}/seller/profile`,
