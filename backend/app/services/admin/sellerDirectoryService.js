...
export async function getActiveSellersData({
  ...
  const baseQuery = { isVerified: true, isActive: true, isSuspended: { $ne: true } };
...
export async function getSuspendedSellersData({
  q = "",
  category = "all",
  sort = "recent",
  page,
  ...
  const baseQuery = { isSuspended: true };
  const filters = [baseQuery];
  ...
  const [sellers, totalSuspendedCount, allSuspendedSellers] = await Promise.all([
    Seller.find(query).lean(),
    Seller.countDocuments(baseQuery),
    Seller.find(baseQuery)
      .select("_id createdAt suspendedAt updatedAt category")
      .lean(),
  ]);

  const sellerIds = sellers.map((seller) => seller._id);
  const allSuspendedSellerIds = allSuspendedSellers.map((seller) => seller._id);
  ...
  const [ordersBySeller, productsBySeller, overallOrderStats] = await Promise.all([
    ...
    sellerIds.length
      ...
      : Promise.resolve([]),
    allSuspendedSellerIds.length
      ? Order.aggregate([
          { $match: { seller: { $in: allSuspendedSellerIds } } },
          {
            $group: {
              _id: null,
              totalOrders: { $sum: 1 },
  ...
  const enrichedSellers = sellers.map((seller) => {
    ...
    return {
      ...
      verificationStatus: seller.isVerified ? "verified" : "unverified",
      isSuspended: true,
      suspendedAt: seller.suspendedAt || seller.updatedAt || null,
  ...
  const suspendedThisMonth = allSuspendedSellers.filter((seller) => {
    const suspendedAt = seller.suspendedAt || seller.updatedAt ? new Date(seller.suspendedAt || seller.updatedAt) : null;
    if (!suspendedAt) {
      return false;
    }
  ...
  return {
    ...
    totalPages: Math.ceil(total / limit) || 1,
    stats: {
      totalSuspendedSellers: totalSuspendedCount,
      totalOrders,
      totalRevenue,
      suspendedThisMonth,
      averageRevenuePerSeller: totalSuspendedCount ? totalRevenue / totalSuspendedCount : 0,
      averageOrdersPerSeller: totalSuspendedCount ? totalOrders / totalSuspendedCount : 0,
    },
    filters: {
      categories: uniqueCategories,
    },
