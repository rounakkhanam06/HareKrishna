...
export const getSuspendedSellers = async (req, res) => {
  try {
    const { q = "", category = "all", sort = "recent" } = req.query;
    const { page, limit, skip } = getPagination(req, {
      defaultLimit: 20,
    ...
    const data = await getSuspendedSellersData({
      q,
      category,
      sort,
      page,
    ...
    return handleResponse(res, 200, "Suspended sellers fetched successfully", data);
  } catch (error) {
    return handleResponse(res, 500, error.message);
  }
};
