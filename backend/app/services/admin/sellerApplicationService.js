...
export async function approveSellerApplicationById({ sellerId, reviewedBy }) {
  ...
  try {
    emitSellerNotification("SELLER_APPROVED", {
      sellerId: seller._id,
    });
  } catch (err) {
    console.error("Failed to notify seller of approval:", err);
  }
...
export async function rejectSellerApplicationById({
  ...
  try {
    emitSellerNotification("SELLER_REJECTED", {
      sellerId: seller._id,
      feedback: reason || "Details provided do not meet platform criteria.",
    });
  } catch (err) {
    console.error("Failed to notify seller of rejection:", err);
  }
