...
export const getProducts = async (req, res) => {
  try {
    ...
    if (isCustomerVisibilityRequest(req)) {
      const activeSellers = await Seller.find({ isActive: true, isSuspended: { $ne: true } })
        .select("_id")
        .lean();
      const activeSellerIds = activeSellers.map((s) => String(s._id));

      if (query.sellerId) {
        if (query.sellerId.$in) {
          const filtered = query.sellerId.$in.map(String).filter((id) => activeSellerIds.includes(id));
          ...
          query.sellerId = { $in: filtered };
        } else {
          const sid = String(query.sellerId);
          if (!activeSellerIds.includes(sid)) {
            return handleResponse(res, 200, "No products available", {
              items: [],
              page: 1,
              limit: 24,
        ...
        query.sellerId = { $in: activeSellerIds };
      }
    }
    ...
    const fetchFn = async () => {
      const [rawProducts, total] = await Promise.all([
        Product.find(finalQuery)
          .select(
            "name slug description sku price salePrice stock brand weight mainImage galleryImages headerId categoryId subcategoryId sellerId status approvalStatus approvalRequestedAt approvalReviewedAt approvalReviewedBy approvalNote lastSubmittedByRole isFeatured variants isReturnable returnWindowDays shelfLife countryOfOrigin fssaiLicense averageRating totalRatings createdAt",
          )
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (files.length > 0) {
      ...
      let keepExisting = [];
      if (productData.keepGalleryImages) {
        try {
          keepExisting = JSON.parse(productData.keepGalleryImages);
        } catch (_) {
          keepExisting = [];
        }
      ...
      if (galleryUrls.length > 0 || keepExisting.length > 0) {
        productData.galleryImages = [...keepExisting, ...galleryUrls];
      } else if (productData.keepGalleryImages === undefined && galleryUrls.length === 0) {
        // No gallery instruction at all — leave existing untouched
      }
    } else {
      // No new files uploaded — still apply keepGalleryImages if sent
      if (productData.keepGalleryImages !== undefined) {
        try {
          productData.galleryImages = JSON.parse(productData.keepGalleryImages);
        } catch (_) {
          productData.galleryImages = [];
        }
        delete productData.keepGalleryImages;
  ...
  try {
    ...
    const product = await getOrSet(
      ...
      async () =>
        Product.findById(id)
          .select(
            "name slug description sku price salePrice stock lowStockAlert brand weight mainImage galleryImages headerId categoryId subcategoryId sellerId status approvalStatus approvalRequestedAt approvalReviewedAt approvalReviewedBy approvalNote lastSubmittedByRole isFeatured variants isReturnable returnWindowDays averageRating totalRatings ratingDistribution totalReviews reviewImagesCount createdAt",
          )
          .populate("headerId", "name")
          .populate("categoryId", "name")
          .populate("subcategoryId", "name")
    ...
    if (enforceRadius) {
      const sellerStatus = await Seller.findById(sellerIdForProduct).select("isActive isSuspended").lean();
      if (!sellerStatus || sellerStatus.isSuspended || !sellerStatus.isActive) {
        return handleResponse(res, 404, "Product not found");
      }
    ...
    return handleResponse(
      ...
      req.user?.id ? { ...normalized, hasPurchased } : normalized,
    );
  } catch (error) {
    return handleResponse(res, 500, error.message);
  }
...
export const getModerationProducts = async (req, res) => {
  try {
    ...
    const [items, total, allCount, pendingCount, approvedCount, rejectedCount,
      globalTotalProducts, globalActiveProducts, globalLowStockProducts, globalOutOfStockProducts] =
      await Promise.all([
        ...
        // Global stats — count across ALL products (no active filters applied)
        Product.countDocuments({}),
    ...
    return handleResponse(res, 200, "Moderation products fetched", {
      ...
      stats: {
        ...
        activeProducts: globalActiveProducts,
        lowStockProducts: globalLowStockProducts,
        outOfStockProducts: globalOutOfStockProducts,
      },
    });
  } catch (error) {
    return handleResponse(res, 500, error.message);
...
export const toggleProductStatus = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error("Cache invalidation error", { scope: "toggleProductStatus", error: cacheErr });
    }

    return handleResponse(res, 200, `Product status updated to ${nextStatus}`, product);
  } catch (error) {
    logger.error("Toggle Product Status Error", { scope: "toggleProductStatus", error });
    return handleResponse(res, 500, error.message);
  }
};

// One-time migration: auto-approve any legacy pending products
Product.updateMany(
  { $or: [{ approvalStatus: "pending" }, { approvalStatus: { $exists: false } }, { approvalStatus: null }] },
  { $set: { approvalStatus: "approved" } }
).then((res) => {
  if (res.modifiedCount > 0) {
    logger.info(`Migrated ${res.modifiedCount} pending products to approved status`);
  }
}).catch(() => {});
