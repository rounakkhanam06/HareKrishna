...
function deliveryBroadcastPayloadFromOrder(order, extra = {}) {
  ...
  return {
    ...
    preview: {
      ...
      basePrice: order.paymentBreakdown?.riderPayoutBase ?? 0,
      tip: order.paymentBreakdown?.riderTipAmount ?? 0,
...
export async function sellerAcceptAtomic(sellerId, orderId) {
  ...
  console.log("[DEBUG Accept] Input params:", { sellerId, orderId });
  const rawDoc = await Order.findOne({ orderId });
  console.log("[DEBUG Accept] Raw document exists:", !!rawDoc);
  if (rawDoc) {
    console.log("[DEBUG Accept] Raw document details:", {
      seller: rawDoc.seller,
      workflowVersion: rawDoc.workflowVersion,
      workflowStatus: rawDoc.workflowStatus,
      paymentMode: rawDoc.paymentMode,
...
export async function sellerRejectAtomic(sellerId, orderId) {
  ...
  console.log("[DEBUG Reject] Input params:", { sellerId, orderId });
  const rawDoc = await Order.findOne({ orderId });
  console.log("[DEBUG Reject] Raw document exists:", !!rawDoc);
  if (rawDoc) {
    console.log("[DEBUG Reject] Raw document details:", {
      seller: rawDoc.seller,
      workflowVersion: rawDoc.workflowVersion,
      workflowStatus: rawDoc.workflowStatus,
    });
...
export async function processSellerTimeoutJob({ orderId }) {
  ...
  const sellerIdStr = updated.seller?.toString();
  if (sellerIdStr) {
...
export async function processDeliveryTimeoutJob({ orderId, attempt }) {
  ...
  const sellerIdStrDT = updated.seller?.toString();
  if (sellerIdStrDT) {
    emitToSeller(sellerIdStrDT, {
      event: "order:cancelled",
      payload: {
        orderId: updated.orderId,
        workflowStatus: WORKFLOW_STATUS.CANCELLED,
        status: "cancelled",
        reason: "No delivery partner available",
      },
    });
  }
