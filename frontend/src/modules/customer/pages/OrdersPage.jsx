...
const OrdersPage = () => {
    ...
    // Real-time: update order status in the list when backend emits a status change
    // (e.g. order auto-cancelled due to seller / delivery accept timeout).
    useEffect(() => {
        ...
        const off = onOrderStatusUpdate(getToken, (payload) => {
            ...
            if (!payload?.orderId || !ws) return;

            // Real-time toast: notify customer instantly when seller times out
            if (ws === 'CANCELLED') {
                toast.error(
                    `Order #${String(payload.orderId).slice(-6)} cancelled — Seller did not respond in time.`,
                    {
                        duration: 6000,
                        id: `order-cancel-${payload.orderId}`, // dedup
                    },
                );
            }
                            ...
                            <div className="flex justify-between items-start gap-3 mb-3.5">
                                <div className="flex gap-3.5 flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        {order.deliveryBoy && (
                                            <p className="mt-1 text-[10px] font-bold text-brand-700 bg-brand-50/50 border border-brand-100/60 rounded px-1.5 py-0.5 inline-flex items-center gap-1">
                                                🚴 {order.deliveryBoy.name} {order.deliveryBoy.phone ? `(${order.deliveryBoy.phone})` : ''}
                                            </p>
                                        )}
                                    </div>
                                </div>
                                <div className="flex flex-col items-end gap-1 shrink-0 max-w-[130px] text-right">
                                    <span
                                        className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.08em] text-center ${
                                            legacy === 'delivered'
                                    ...
                                    >
                                        <span className="flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white/80 shrink-0">
                                            <CheckCircle
                                                size={9}
                                                className={`${
                                                    legacy === 'delivered'
                                        ...
                                        <span className="leading-tight">{getOrderStatusLabel(order).toUpperCase()}</span>
                                    </span>
