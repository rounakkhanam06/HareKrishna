...
if (!firebaseConfig.apiKey || !firebaseConfig.projectId || firebaseConfig.apiKey.startsWith("__")) {
  ...
  messaging.onBackgroundMessage(async (payload) => {
    // Swiggy / Zomato Pattern: Only show OS notification bar alert when no active window is focused
    const clientList = await self.clients.matchAll({ type: "window", includeUncontrolled: true });
    const hasFocusedWindow = clientList.some((client) => client.focused || client.visibilityState === "visible");

    if (hasFocusedWindow) {
      // App is open in foreground — in-app socket handles UI, suppress system push bar popup
      return;
    }
