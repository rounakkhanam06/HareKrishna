...
export async function settleRiderCashEntry({ riderId, amount, method }) {
  ...
  for (const tx of transactions) {
    if (tx.type === "Cash Collection") {
      currentCash += (tx.amount || 0);
    } else {
      currentCash -= Math.abs(tx.amount || 0);
    }
  }

  if (amount > currentCash) {
    throw new Error(`Settlement amount cannot exceed the rider's current cash in hand (₹${currentCash})`);
  }
